...
namespace YAML
{
	...
	template <>
	struct convert<CmHazel::UUID>
	{
		static Node encode(const CmHazel::UUID& uuid)
		{
			Node node;
		...
		static bool decode(const Node& node, CmHazel::UUID& uuid)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)             \
	...
	static std::string RigidBody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
	{
		switch (bodyType)
		{
		case CmHazel::Rigidbody2DComponent::BodyType::Static: 			return "Static";
		case CmHazel::Rigidbody2DComponent::BodyType::Dynamic:			return "Dynamic";
		case CmHazel::Rigidbody2DComponent::BodyType::Kinematic:		return "Kinematic";
		}
	...
	static void SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		if (entity.HasComponent<ScriptComponent>())
		{
			...
			// Fields
			Shared<ScriptClass> entityClass = ScriptEngine::GetEntityClass(scriptComponent.ClassName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				if (scriptComponent)
				{
					...
					if (scriptFields)
					{
						Shared<ScriptClass> entityClass = ScriptEngine::GetEntityClass(sc.ClassName);
						CM_CORE_ASSERT(entityClass);
						const auto& fields = entityClass->GetFields();
						...
						for (auto scriptField : scriptFields)
						{
							...
							CM_CORE_ASSERT(fields.find(name) != fields.end());
