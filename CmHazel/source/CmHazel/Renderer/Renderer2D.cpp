...
#include "CmHazel/Renderer/UniformBuffer.h"
...
namespace CmHazel
{
	...
	struct Renderer2DData
	{
		...
		Shared<Texture2D> FontAtlasTexture;
	...
	void Renderer2D::Init()
	{
		...
		s_Data.CircleVertexArray->SetIndexBuffer(quadIB); // 使用 quad IB
	...
	void Renderer2D::DrawQuad(const glm::mat4& transform, const Shared<Texture2D>& texture, float tilingFactor, const glm::vec4& tintColor, int entityID)
	{
		CM_PROFILE_FUNCTION();
	...
	void Renderer2D::DrawRotatedQuad(const glm::vec2& position, const glm::vec2& size, float rotation, const Shared<Texture2D>& texture, float tilingFactor, const glm::vec4& tintColor)
	{
		DrawRotatedQuad({ position.x, position.y, 0.0f }, size, rotation, texture, tilingFactor, tintColor);
	}

	void Renderer2D::DrawRotatedQuad(const glm::vec3& position, const glm::vec2& size, float rotation, const Shared<Texture2D>& texture, float tilingFactor, const glm::vec4& tintColor)
	{
		CM_PROFILE_FUNCTION();
	...
	{
		const auto& fontGeometery = font->GetMSDFData()->FontGeometry;
		const auto& metrics = fontGeometery.getMetrics();
		...
		const float spaceGlyphAdvance = fontGeometery.getGlyph(' ')->getAdvance();
		...
		for (size_t i = 0; i < string.size(); i++)
		{
			...
			if (character == ' ')
			{
				...
				if (i < string.size() - 1)
				{
					...
					double dAdcance;
					fontGeometery.getAdvance(dAdcance, character, nextCharacter);
					advance = (float)dAdcance;
				}
			...
			auto glyph = fontGeometery.getGlyph(character);
			if (!glyph)
				glyph = fontGeometery.getGlyph('?');
			if (!glyph)
				return;
			...
			if (i < string.size() - 1)
			{
				...
				char nextCharacter = string[i + 1];
				fontGeometery.getAdvance(advance, character, nextCharacter);
