...
namespace CmHazel
{
	...
	static std::unordered_map<std::string, ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "CmHazel.Vector2", ScriptFieldType::Vector2 },
		{ "CmHazel.Vector3", ScriptFieldType::Vector3 },
		...
		static MonoAssembly* LoadMonoAssembly(const std::filesystem::path& assemblyPath, bool loadPDB = false)
		{
			...
			if (loadPDB)
			{
				...
				if (std::filesystem::exists(pdbPath))
				{
					...
					CM_CORE_INFO("Loaded PDB {}", pdbPath.string());
					delete[] pdbFileData;
				}
			}
	...
	void ScriptEngine::InitMono()
	{
		...
		if (s_Data->EnableDebugging)
		{
			...
			mono_debug_init(MONO_DEBUG_FORMAT_NONE);
		}
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			// 这个例程是一个用于检索类中字段的迭代器例程。
			// 你必须传入一个指向零的 gpointer，它被视为一个不透明句柄，用于迭代所有元素。
			// 当没有更多值可用时，返回值为 NULL。
			int fieldCount = mono_class_num_fields(monoClass);
			CM_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					CM_CORE_WARN("{} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
