...
#define CM_ADD_INTERNAL_CALL(Name) mono_add_internal_call("CmHazel.InternalCalls::" #Name, Name);
	...
	static bool Entity_HasComponent(UUID entityID, MonoReflectionType* componentType)
	{
		...
		CM_CORE_ASSERT(s_EntityHasComponentFuncs.find(managedType) != s_EntityHasComponentFuncs.end());
	...
	static MonoString* TextComponent_GetText(UUID entityID)
	{
		...
		Entity entity = scene->GetEntityByUUID(entityID);
		CM_CORE_ASSERT(entity);
		CM_CORE_ASSERT(entity.HasComponent<TextComponent>());
		...
		([]()
			{
				...
				std::string managedTypename = fmt::format("CmHazel.{}", structName);

				MonoType* managedType = mono_reflection_type_from_name(managedTypename.data(), ScriptEngine::GetCoreAssemblyImage());
				if (!managedType)
				{
					CM_CORE_ERROR("Could not find component type {}", managedTypename);
					return;
				}
	...
	void ScriptGlue::RegisterFunctions()
	{
		...
		CM_ADD_INTERNAL_CALL(GetScriptInstance);

		CM_ADD_INTERNAL_CALL(Entity_HasComponent);
		CM_ADD_INTERNAL_CALL(Entity_FindEntityByName);
		...
		CM_ADD_INTERNAL_CALL(TextComponent_GetText);
		CM_ADD_INTERNAL_CALL(TextComponent_SetText);
		CM_ADD_INTERNAL_CALL(TextComponent_GetColor);
		CM_ADD_INTERNAL_CALL(TextComponent_SetColor);
		CM_ADD_INTERNAL_CALL(TextComponent_GetKerning);
		CM_ADD_INTERNAL_CALL(TextComponent_SetKerning);
		CM_ADD_INTERNAL_CALL(TextComponent_GetLineSpacing);
		CM_ADD_INTERNAL_CALL(TextComponent_SetLineSpacing);
