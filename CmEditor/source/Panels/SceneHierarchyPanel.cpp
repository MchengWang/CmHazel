		...
		if (ImGui::BeginPopup("AddComponent"))
		{
			if (ImGui::MenuItem("Camera"))
			{
				if (!m_SelectionContext.HasComponent<CameraComponent>())
					...
					CM_CORE_WARN("This entity already has the Camera Component!");
			...
			if (ImGui::MenuItem("Sprite Renderer"))
			{
				if (!m_SelectionContext.HasComponent<SpriteRendererComponent>())
					...
					CM_CORE_WARN("This entity already has the Sprite Renderer Component!");
				ImGui::CloseCurrentPopup();
			}
		...
		DrawComponent<CameraComponent>("Camera", entity, [](auto& component)
			{
				...
				const char* currentProjecrionTypeString = projectionTypeStrings[(int)camera.GetProjectionType()];
				if (ImGui::BeginCombo("Projection", currentProjecrionTypeString))
				{
					for (int i = 0; i < 2; i++)
					{
						bool isSelected = currentProjecrionTypeString == projectionTypeStrings[i];
						if (ImGui::Selectable(projectionTypeStrings[i], isSelected))
						{
							currentProjecrionTypeString = projectionTypeStrings[i];
				...
				if (camera.GetProjectionType() == SceneCamera::ProjectionType::Orthographic)
				{
					...
					float orthoNear = glm::degrees(camera.GetOrthographicNearClip());
					if (ImGui::DragFloat("Near", &orthoNear))
						camera.SetOrthographicNearClip(glm::radians(orthoNear));

					float orthoFar = glm::degrees(camera.GetOrthographicFarClip());
					if (ImGui::DragFloat("Far", &orthoFar))
						camera.SetOrthographicFarClip(glm::radians(orthoFar));
